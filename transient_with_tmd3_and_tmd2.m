...
% Define time vector
tspan = 0:0.001:120; % 120 seconds with 1 ms timestep
...
F_interp = @(t) [interp1(t_data, f0, t, 'linear', 0); 0; 0; 0; 0]; 
...
plot(t_sol, x_disp(:,4), 'y', 'LineWidth',1.5)
plot(t_sol, x_disp(:,5), 'p', 'LineWidth',1.5)
xlabel('Time (s)')
ylabel('Displacement (m)')
title('Transient Response of 3-Floor Building')
legend('Floor 1','Floor 2','Floor 3', 'TMD3', 'TMD2')
grid on
...
plot(t_sol, x_acc(:,4), 'y', 'LineWidth',1.5)
plot(t_sol, x_acc(:,5), 'p', 'LineWidth',1.5)
xlabel('Time (s)')
